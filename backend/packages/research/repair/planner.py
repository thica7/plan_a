...
def repair_tasks_from_gaps(gaps: list[QualityGap]) -> list[RepairTask]:
    return [_repair_task_from_gap(gap) for gap in gaps if gap.severity in {"warn", "blocker"}]


def _repair_task_from_gap(gap: QualityGap) -> RepairTask:
    target_fields = _target_fields(gap.field)
    return RepairTask(
        gap_id=gap.id,
        ...
        query_hints=_query_hints(gap, target_fields),
        acceptance_rule=gap.acceptance_rule,
        metadata={"gap_reason": gap.reason, "severity": gap.severity},
    )


def _query_hints(gap: QualityGap, fields: list[str]) -> list[str]:
...
def _target_fields(field: str | None) -> list[str]:
    if not field:
        return []

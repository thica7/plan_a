...
async function createSitemapSource(url: string, config: Record<string, unknown>) {
...
export function SitemapTab({ competitor, dimension, priority, onSubmitted }: SitemapTabProps) {
  ...
  const loadPreview = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      setPreview(parseSitemapUrls(await res.text()).slice(0, 10));
    } catch (err) {
      setPreview([]);
  ...
  const submit = async () => {
    ...
    setError(null);
    try {
      await createSitemapSource(url.trim(), { competitor, dimension, priority, max_urls: maxUrls });
      setUrl('');
      setPreview([]);
      onSubmitted();
    } catch (err) {
  ...
  return (
    <div className="grid gap-3">
      ...
      <button type="button" className="btn btn-primary btn-sm w-fit" onClick={submit} disabled={submitting || !url.trim()}>
        {submitting ? t('crawl.submitting') : t('crawl.submitSitemap')}
      </button>
    </div>
  );
}
